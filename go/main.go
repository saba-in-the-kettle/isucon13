...
func initializeHandler(c echo.Context) error {
	...
	if err := isuutil.CreateIndexIfNotExists(dbConn, "create index livestreams_user_id_index\n    on livestreams (user_id);\n\n"); err != nil {
		c.Logger().Errorf("create livestreams_user_id_index failed with err=%s", err)
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to initialize: "+err.Error())
	}
	if err := isuutil.CreateIndexIfNotExists(dbConn, "create index livecomments_livestream_id_index\n    on livecomments (livestream_id);\n\n"); err != nil {
		c.Logger().Errorf("create livestreams_user_id_index failed with err=%s", err)
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to initialize: "+err.Error())
	}
	if err := isuutil.CreateIndexIfNotExists(dbConn, "create index ng_words_livestream_id_index\n    on ng_words (livestream_id);\n\n"); err != nil {
		c.Logger().Errorf("create livestreams_user_id_index failed with err=%s", err)
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to initialize: "+err.Error())
	}

	if err := isuutil.CreateIndexIfNotExists(dbConn, "create index reservation_slots_start_at_index\n    on reservation_slots (start_at);\n\n"); err != nil {
		c.Logger().Errorf("create reservation_slots_start_at_index failed with err=%s", err)
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to initialize: "+err.Error())
	}
